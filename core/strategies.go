...
type StrategyI interface {
	GetType() string
	UnrelayedSequences(src, dst *Chain, sh SyncHeadersI) (*RelaySequences, error)
	RelayPackets(src, dst *Chain, sp *RelaySequences, sh SyncHeadersI) error
	UnrelayedAcknowledgements(src, dst *Chain, sh SyncHeadersI) (*RelaySequences, error)
	RelayAcknowledgements(src, dst *Chain, sp *RelaySequences, sh SyncHeadersI) error
}
...
func RunStrategy(src, dst *Chain, strategy StrategyI) (func(), error) {
